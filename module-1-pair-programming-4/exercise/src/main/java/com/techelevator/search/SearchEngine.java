...
//import static jdk.nashorn.tools.jjs.JavacPackagesHelper.getFiles;
...
//		SearchDomain index = new SearchDomain("./exercise/data");
		int fileID = 0;
		String line = "";

		for(String fileName : sd.getFiles()){
			try(Scanner indexFile = new Scanner(new File(sd.getFolder() + "/" + fileName))) {
				while (indexFile.hasNextLine()) {
					line = indexFile.nextLine();
				...
				throw new SearchEngineException(("Exception: File " + fileName + " not found."));
			}
...
//		for(int i = 0; i < index.getFiles().size() -1; i++){
//			try(Scanner fileInput = new Scanner(index.getFiles().get(i))){
//				while (fileInput.hasNextLine()){
//					String line = fileInput.nextLine();
//					indexWords(i, line);
//				}
//			} catch (Exception e){
//				System.out.println("The file was not found: " + index.getFiles().get(i));
//			}
...
//		int fileID;
//
//		for (fileID = 0; fileID < sd.getFiles().size(); fileID++){
//			String fileName = sd.getFiles().get(fileID);
